...
class YoutubeCrawler(WebDriver):
    LIKES = "LIKES"
    DISLIKES = "DISLIKES"
    VIEWS = "VIEWS"
    UPLOAD_DATE = "UPLOAD_DATE"
    ...
    def __init__(self, webdriver):
        ...
        self.__df__ = pd.DataFrame(
            columns=[
                CSV_COLS.URL,
                CSV_COLS.TITLE,
                CSV_COLS.DESCRIPTION,
                CSV_COLS.LIKES,
                CSV_COLS.DISLIKES,
                CSV_COLS.VIEWS,
                CSV_COLS.UPLOAD_DATE,
                CSV_COLS.DURATION,
                CSV_COLS.NUM_COMMENTS,
                CSV_COLS.CHANNEL_NAME,
                CSV_COLS.CHANNEL_SUBS,
                CSV_COLS.URL,
                CSV_COLS.TRANSCRIPT,
            ]
        )
    ...
    def get_video_transcript(self):
        ...
        transcript = self.driver.get_multiple_elements(
            X_PATH_CONSTANTS.TRANSCRIPT.TRANSCRIPT
        )
        ...
        out_dict = {
            CSV_COLS.URL: self.url,
            CSV_COLS.TITLE: title,
            CSV_COLS.DESCRIPTION: description,
            CSV_COLS.LIKES: likes,
            CSV_COLS.DISLIKES: dislikes,
            CSV_COLS.VIEWS: views,
            CSV_COLS.UPLOAD_DATE: upload_date,
            CSV_COLS.DURATION: duration,
            CSV_COLS.NUM_COMMENTS: num_comments,
            CSV_COLS.CHANNEL_NAME: channel_name,
            CSV_COLS.CHANNEL_SUBS: channel_subs,
            CSV_COLS.URL: self.url,
            CSV_COLS.TRANSCRIPT: transcript,
        }
    ...
    def get_all_comments_from_video(self):
        ...
        total_comments = self.driver.get_element_text(
            X_PATH_CONSTANTS.COMMENTS.NUMBER_COMMENTS
        )
        ...
        number_comments_shown = len(
            self.driver.get_multiple_elements(X_PATH_CONSTANTS.COMMENTS.COMMENT_BLOCK)
        )

        # we need to keep scrolling until all of the comments are lazy loaded in.
        while total_comments != number_comments_shown:
            number_comments_shown = len(
                self.driver.get_multiple_elements(
                    X_PATH_CONSTANTS.COMMENTS.COMMENT_BLOCK
                )
            )  # now we can collect the information
            ...
            if new_position == old_position:
                break
        comment_blocks = self.driver.get_multiple_elements(
            X_PATH_CONSTANTS.COMMENTS.COMMENT_BLOCK
        )
        ...
        parsed_comment_blocks = list(
            map(lambda comment: self.convert_comment(comment), parsed_comment_blocks)
        )
